...
.PHONY: help venv up down logs migrate seed test lint typecheck preflight probe \
        ...
        gateway-ingest gateway-report compare-recognisers deploy-secrets gap-report
...
gap-report:  ## Sample gap-analysis report (Model 1 deliverable) into reports/
	$(BE) ../$(PY) scripts/gap_report.py --emit-evidence
