...
| 12 | Screenshots of the deployed instance | `docs/screenshots/` | ✅ **the same set as row 11** — every image there is captured against https://setu-gujarat.netlify.app with the data that is in it. The separate `deployed/` folder is gone: its eight images were from 27 August, taken against the container stack before the estate moved host, before the cameras were geocoded, and before Control Room, Zones, Demo and the light theme existed. A folder captioned "the deployed instance" showing a fortnight-old one is worse than no folder |
| 13 | Hosted URL + test credentials | **Console: https://setu-gujarat.netlify.app** · API: https://setu-api-ai7z.onrender.com | ✅ **live**, verified 10/10 by `verify_deployment.py`. Deployed on Render + Netlify, not Railway — see `docs/adr/0004` addendum. Credentials in `deploy-secrets.env` (gitignored) |
...
- [x] ~~Deploy and record the live URL with credentials~~ — live at
      https://setu-gujarat.netlify.app, verified 10/10. Credentials go on the submission
